...
function inlinerCoreSelectNextSentence(doc) {
    ...
    try {
        ...
        if (FExtension.browserInject.getBrowserName() == 'firefox' && sentence == "") {
            return -1;
        }
        var sel = FExtension.browserInject.modifySentence(selection, doc, true);

        if (FExtension.browserInject.getBrowserName() == 'firefox' && sel == sentence) {//sentence.length == selection.toString().trim().length + 2 &&
            doc.arraySentence.splice(0, 1);
            if (doc.arraySentence && doc.arraySentence.length == 1 && doc.arraySentence[0] == "") {
...
tagsBreak = ['DIV', 'P', 'H1', 'H2', 'H3', 'H4', 'H5', 'H6', 'BR', 'UL', 'LI', 'SPAN'];
...
//            console.log(e);
 ...
 try{
    ...
    var url_ = String(document.location);

    if(url_.indexOf('.facebook.')!=-1){
	    var cnt=0;
	    while (!selection.toString().match(/\S/)) {
	            if(cnt<10){
        		selection.modify('extend', 'backward', 'character');
...
function snapSelectionToWord(doc) {
 try{
    ...
    if (window.getSelection && (selection = window.getSelection()).modify) {
        ...
        var tmp = String(selection).split(" ");

	if(tmp.length>1) clearWhitespase(doc);
