ImTranslator_inliner = {
	...
	init: function(){
		//********************Options for inliner****************************
		if (!FExtension.GET_localStorage("SL_langSrc_it")) { FExtension.store.set("SL_langSrc_it", "auto"); chrome.storage.local.set({"SL_langSrc_it": "auto"});}
		if (!FExtension.GET_localStorage("SL_langDst_it")) { FExtension.store.set("SL_langDst_it", FExtension.store.SL_BR_LN); chrome.storage.local.set({"SL_langDst_it": FExtension.store.SL_BR_LN});}


		if (!FExtension.GET_localStorage("SL_shortcutInliner")) { FExtension.store.set("SL_shortcutInliner", "T"); chrome.storage.local.set({"SL_shortcutInliner": "T"});}
		if (!FExtension.GET_localStorage("SL_shortcutClean")) { FExtension.store.set("SL_shortcutClean", "X"); chrome.storage.local.set({"SL_shortcutClean": "X"});}
		if (!FExtension.GET_localStorage("SL_style")) { FExtension.store.set("SL_style", "239e23"); chrome.storage.local.set({"SL_style": "239e23"});}
		if (!FExtension.GET_localStorage("SL_inject_before")) { FExtension.store.set("SL_inject_before", "false"); chrome.storage.local.set({"SL_inject_before": "false"});}
		if (!FExtension.GET_localStorage("SL_inject_brackets")) { FExtension.store.set("SL_inject_brackets", "true"); chrome.storage.local.set({"SL_inject_brackets": "true"});}
		if (!FExtension.GET_localStorage("SL_line_break")) { FExtension.store.set("SL_line_break", "false"); chrome.storage.local.set({"SL_line_break": "false"});}
		if (!FExtension.GET_localStorage("SL_whole_word")) { FExtension.store.set("SL_whole_word", "true"); chrome.storage.local.set({"SL_whole_word": "true"});}
		if (!FExtension.GET_localStorage("SL_hide_translation")) { FExtension.store.set("SL_hide_translation", "false"); chrome.storage.local.set({"SL_hide_translation": "false"});}

//		if (!FExtension.GET_localStorage("SL_dictionary")) { FExtension.store.set("SL_dictionary", "false"); chrome.storage.local.set({"SL_dictionary": "false"});}
		...
		/*
                ImTranslatorBG.the_ID6 = FExtension.browser.createContextMenus("Inline Translator: "+FExtension.element(FExtension.GET_localStorage("SL_LOCALIZATION"),'extCMTransSel')+" "+ ImTranslatorBG.Lexicon(ImTranslatorBG.GetLongLanguageName(FExtension.GET_localStorage("SL_langDst_it"))),ImTranslator_inliner.inlinerContextOnClick, true);

                    ImTranslatorBG.the_ID7 = FExtension.browser.createContextMenus(FExtension.element(FExtension.GET_localStorage("SL_LOCALIZATION"),'extCMsot'),ImTranslator_inliner.inlinerContextShowOnClick, false);
                if(FExtension.GET_localStorage("SL_hide_translation")!="true")	FExtension.browser.removeContextMenus(ImTranslatorBG.the_ID7);


                ImTranslatorBG.the_ID8 = FExtension.browser.createContextMenus(FExtension.element(FExtension.GET_localStorage("SL_LOCALIZATION"),'extCMct'),ImTranslator_inliner.inlinerContextClearOnClick, false);
                ImTranslatorBG.the_ID9 = FExtension.browser.createContextMenus(FExtension.element(FExtension.GET_localStorage("SL_LOCALIZATION"),'extCMcl'),ImTranslatorBG.SL_SET_TRANSLATION_LNG, false);
        */
	...
	translate: function (text, injectDictionary) {
		...
		var langDst = FExtension.GET_localStorage("SL_langDst_it");
	...
	respondToMessage: function(theMessageEvent) {
		if(theMessageEvent.name === "inlinerSelectionRequest") {
			...
			setTimeout(function(){
				chrome.storage.local.set({"SL_dictionary": ""});
			...
			FExtension.browser.inlinerPostMessage({name:"shortcutInlinerSelectionValue", message:FExtension.GET_localStorage("SL_FK_box1")+"|"+FExtension.GET_localStorage("SL_inlinerFK1")+"|"+FExtension.GET_localStorage("SL_shortcutInliner")});
		} else if (theMessageEvent.name === "shortcutInlinerCleanRequest") {
			FExtension.browser.inlinerPostMessage({name:"shortcutInlinerCleanValue", message:FExtension.GET_localStorage("SL_FK_box2")+"|"+FExtension.GET_localStorage("SL_inlinerFK2")+"|"+FExtension.GET_localStorage("SL_shortcutClean")});
		} else if (theMessageEvent.name === "styleDestinationRequest") {
			FExtension.browser.inlinerPostMessage({name:"styleDestinationValue", message:FExtension.GET_localStorage("SL_style")});
		} else if (theMessageEvent.name === "injectBeforeRequest") {
			FExtension.browser.inlinerPostMessage({name:"injectBeforeValue", message:FExtension.GET_localStorage("SL_inject_before")});
		} else if (theMessageEvent.name === "injectBracketsRequest") {
			FExtension.browser.inlinerPostMessage({name:"injectBracketsValue", message:FExtension.GET_localStorage("SL_inject_brackets")});
		} else if (theMessageEvent.name === "injectLineBreak") {
			FExtension.browser.inlinerPostMessage({name:"injectLineBreak", message:FExtension.GET_localStorage("SL_line_break")});
		} else if (theMessageEvent.name === "injectWholeWord") {
			FExtension.browser.inlinerPostMessage({name:"injectWholeWord", message:FExtension.GET_localStorage("SL_whole_word")});
		} else if (theMessageEvent.name === "injectHideTranslation") {
			FExtension.browser.inlinerPostMessage({name:"injectHideTranslation", message:FExtension.GET_localStorage("SL_hide_translation")});
		} else if (theMessageEvent.name === "injectDictionary") {
			FExtension.browser.inlinerPostMessage({name:"injectDictionary", message:FExtension.GET_localStorage("SL_dictionary")});
			...
			FExtension.browser.inlinerPostMessage({name:"injectLangSrc", message:FExtension.GET_localStorage("SL_langSrc_it")});
		} else if (theMessageEvent.name === "injectLangDst") {
			FExtension.browser.inlinerPostMessage({name:"injectLangDst", message:FExtension.GET_localStorage("SL_langDst_it2")});
		}
	},
	...
	DODetectionLang: function(myTransText) {
		var SL_langSrc = FExtension.GET_localStorage("SL_langSrc");
		var AUTO = SL_langSrc;
		if(AUTO=="auto"){
			if(myTransText!=""){
		...
		try{
			chrome.windows.getAll(null, function(wins) {
				for (var j = 0; j < wins.length; ++j) {
					chrome.tabs.query(wins[j].id, function(tabs) {
						var theMessageEvent = {};
