...
(function(){var loc=GEBI("SLlocpl");loc.addEventListener("click",function(){GEBI("SL_DETECTED").style.display="none";LOCcontrol();SL_DICT();},!1);} )();
...
//   SL_DETECT="";
   ...
   if(SL_lng=="ar" || SL_lng=="iw" || SL_lng=="fa" || SL_lng=="ur" || SL_lng=="yi" || SL_lng=="ps"  || SL_lng=="sd" || SL_lng=="ckb"){
...
function SetProperHeight____(h){
	var h = window.innerHeight-h+50;
	GEBI("SL_DICTsource").style.minHeight = "20px";
	GEBI("SL_DICTsource").style.height = h+"px";
}
...
// text=text.replace(/\"/gi,"");
 ...
 chrome.storage.local.set({'SL_SavedText_gt': decodeURIComponent(text)});
 ...
 if(text!=""){
  ...
  GEBI('SL_DICTtext').style.direction="ltr";
  GEBI('SL_DICTtext').style.textAlign="left";
  if(GEBI('SL_langSrc').value=="ar" || GEBI('SL_langSrc').value=="iw" || GEBI('SL_langSrc').value=="fa" || GEBI('SL_langSrc').value=="ur" || GEBI('SL_langSrc').value=="yi" || GEBI('SL_langSrc').value=="ps" || GEBI('SL_langSrc').value=="sd" || GEBI('SL_langSrc').value=="ckb"){
  	GEBI('SL_DICTtext').style.direction="rtl";
	GEBI('SL_DICTtext').style.textAlign="right";
  }
  ...
  var text = GEBI('SL_DICTtext').value;
	...
	FExtension.bg.ImTranslatorBG.SL_callbackRequest2();
		...
		if(SL_DETECT!="auto"){
			SrcLng = SL_DETECT;
			if(SL_DETECT==GEBI('SL_langDst').value && GEBI('SL_langSrc').value!="auto"){ DstLng = GEBI('SL_langSrc').value;}
//			if(SL_DETECT==GEBI('SL_langDst').value) DstLng = GEBI('SL_langSrc').value;
	...
	var theTEXT=GEBI('SL_DICTtext').value.replace(/\"/ig,"'");
	theTEXT=theTEXT.replace(/\^/ig,"%");
	GEBI('SL_DICTtext').value=theTEXT;
...
//        baseUrl = baseUrl + "?" + SL_Params;
	...
	if(ajaxRequest.readyState == 4){
		...
		if(resp.indexOf('trans":')!=-1){
			...
			if(GEBI('SL_langDst').value=="ar" || GEBI('SL_langDst').value=="iw" || GEBI('SL_langDst').value=="fa" || GEBI('SL_langDst').value=="ur" || GEBI('SL_langDst').value=="yi" || GEBI('SL_langDst').value=="ps" || GEBI('SL_langDst').value=="sd" || GEBI('SL_langDst').value=="ckb"){
			 ...
			 if(SL_WRONGLANGUAGEDETECTED==0 )SET_PROV();
        	         SET_FIRST_AVAILABLE_PROV();
		...
		if(resp.indexOf("<div ")==-1) {			
			var CleanResp = resp.replace(/(<([^>]+)>)/ig,"");
			ForHistory = CleanResp;
			ForHistory = PseudoDICT(ForHistory);
		} 

		if(z==0) resp = PseudoDICT(resp.replace(/(<([^>]+)>)/ig,""));

                if(CleanResp=="" || resp.indexOf("<h1>Not Found</h1>")>-1) SL_OTHER_PROV();
		else{
			  ...
			  SLnow=SLnow.toString();
		          ...
		          var LNGfrom = GEBI("SL_langSrc").value;
                	  if(GEBI("SL_langSrc").value=="auto" || localStorage["SL_no_detect"]=="true" ) LNGfrom = SL_DETECT;
                        	  setTimeout(function(){
				         var URL = "{empty}"; //localStorage["THE_URL"];
					 ...
					 localStorage["THE_URL"]="";
                        		 var txt = GEBI('SL_DICTtext').value;
                                         txt=txt.replace(/~/ig," ");
       	                                 ...
       	                                 var BeforeSanitization = txt + "~~" + ForHistory + "~~" + LNGfrom + "|" + GEBI("SL_langDst").value + "~~"+ URL +"~~"+CurDT+"~~"+HISTORYtype+"^^" + localStorage["SL_History"];
...
function SL_TTSicn(lng,st){
 ...
 if(lng!="ar" && lng!="iw" && lng!="fa" && lng!="ur" && lng!="yi" && lng!="ps" && lng!="sd" && lng!="ckb"){
  if(st==0){
   GEBI("SL_DICTtext").style.direction="ltr";
   GEBI("SL_DICTtext").style.textAlign="left";
  }
...
function SL_ALIGNER1(SL_to){
 ...
 if(SL_to!="ar" && SL_to!="iw" && SL_to!="fa" && SL_to!="ur" && SL_to!="yi" && SL_to!="ps" && SL_to!="sd" && SL_to!="ckb"){
...
function SL_ALIGNER2(SL_from){
 var nums=document.getElementsByTagName("div").length;
 if(SL_from!="ar" && SL_from!="iw" && SL_from!="fa" && SL_from!="ur" && SL_from!="yi" && SL_from!="ps" && SL_from!="sd" && SL_from!="ckb"){
      for(var I = 0; I < nums; I++){
...
function SL_ALIGNER3(SL_from,SL_to){
 if(SL_to=="ar" || SL_to=="iw" || SL_to=="fa" || SL_to=="ur" || SL_to=="yi" || SL_to=="ps" || SL_to=="sd" || SL_to=="ckb")	GEBI("SL_DICTsource").style.textAlign='right';
 else	GEBI("SL_DICTsource").style.textAlign='left';
 if(SL_from=="ar" || SL_from=="iw" || SL_from=="fa" || SL_from=="ur" || SL_from=="yi" || SL_from=="ps" || SL_from=="sd" || SL_from=="ckb")	GEBI("SL_DICTtext").style.textAlign='right';
...
function SL_ALIGNER(SL_to){
 var nums=document.getElementsByTagName("div").length;
 if(SL_to!="ar" && SL_to!="iw" && SL_to!="fa" && SL_to!="ur" && SL_to!="yi" && SL_to!="ps" && SL_to!="sd" && SL_to!="scb"){
      for(var I = 0; I < nums; I++){
	...
	ajaxRequest.onreadystatechange = function(){
		...
		if(ajaxRequest.readyState == 4 && ajaxRequest.status == 200){
			...
			if (theQ==1) CNTR('1332',dir.replace("-","/"), text.length);
			     	...
			     	if(SL_lng2=="ar" || SL_lng2=="iw" || SL_lng2=="fa" || SL_lng2=="ur" || SL_lng2=="yi" || SL_lng2=="ps" || SL_lng2=="sd" || SL_lng2=="ckb"){
			  		 GEBI('SL_DICTsource').style.direction="rtl";
				 	 GEBI('SL_DICTsource').style.textAlign="right";
			     	}
