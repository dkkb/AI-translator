...
//					myPort.postMessage({greeting: "wpt1:>"+url});
				}
	                }
			//WPT MO
...
//					var myPort = browser.runtime.connect({name:"content"});
//					myPort.postMessage({greeting: "wpt2:>"+url});
				}
	                }
			//OPT
...
//					window.open(FExtension.browserInject.getURL('options.html', true),"ImTranslator:Option");
					var myPort = browser.runtime.connect({name:"content"});
...
//			DBHOTKEYSline2=DBHOTKEYSline2.replace(/ \+ /g,":|");
			...
			if(INLINER_ONOFF=="true"){
				if(TranslatorIM.SL_SYNC_FILTER(DBHOTKEYSline3,HOTKEYSline)==true){
					if(TranslatorIM.SL_ONETIMERUN==0) {inlinerInjectInliner();TranslatorIM.SL_ONETIMERUN=1;}
			        }
			}				
		    }	
		  }catch(e){
...
//			  TranslatorIM.SAVE_SES_PARAMS();
	 ...
	 SL_JUMP: function (doc){
		...
		TranslatorIM.SL_bring_DOWN();
		var SLdivField = doc.getElementById("SL_shadow_translator");
		if(doc.getElementById("SL_pin").style.background.indexOf("pin-on")!=-1) TranslatorIM.SL_NEST="FLOAT";
...
//			}
	...
	SL_IT_retranslate: function(){
	     	var doc = FExtension.browserInject.getDocument();
		var id = TranslatorIM.SL_EVENT.target.id;
	...
	getY_TRANSLATION: function(dir, text){
		...
		ajaxRequest.onreadystatechange = function(){
			...
			if(ajaxRequest.readyState == 4 && ajaxRequest.status == 200){
				...
				if(T == "ar" || T == "iw" || T == "fa" || T == "ur" || T == "yi" || T == "ps" || T == "sd"){
					doc.getElementById('SL_shadow_translation_result').style.direction = "rtl";
					doc.getElementById('SL_shadow_translation_result2').style.textAlign = "right";
				}
                                doc.getElementById('SL_loading').style.display="none";
                                TranslatorIM.SL_JUMP(doc);
	...
	SL_SAVE_FAVORITE_LANGUAGES: function(ln, ob, st, TR, TP){
		...
		if(st == 0){
			if(OUT!="") TranslatorIM.SL_REBUILD_TARGET_LANGUAGE_MENU(OUT, ob);	
		}
	},
