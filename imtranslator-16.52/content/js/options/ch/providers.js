(function( name, factory ) {
   ...
   Factory.prototype = {
      ...
      _makeDragItem: function( item )
      {
         ...
         this._dragItem.innerHTML = DOMPurify.sanitize(item.innerHTML); 
...
//         this._dragItem.style["border"] = "1px " + abg[theID] + " solid";
      ...
      _onRelease: function( e )
      {
         this._dragging = false;
         this._trashDragItem(); 
         if(e.target.className == "dragging")  setTimeout(function() {save_options();}, 180);	        
      },
...
function PR_BUILDER(st){
        var dics = FExtension.GET_localStorage("SL_DICT_PRESENT");
	var arr = dics.split(",");

        var defaultset = localStorage[st];

	if(defaultset==""){
                defaultset = "Google,Microsoft,Translator,Yandex";
		FExtension.store.set("SL_ALL_PROVIDERS_GT", defaultset);
		chrome.storage.local.set({'SL_ALL_PROVIDERS_GT': defaultset});
		FExtension.store.set("SL_ALL_PROVIDERS_BBL", defaultset);
		chrome.storage.local.set({'SL_ALL_PROVIDERS_BBL': defaultset});
        }
...
//	var TTS = "Google,Translator";
	...
	build = build + "<div style='margin-left:10px;'><img src='../../../../content/img/util/dictionary.png'> " + FExtension.element(FExtension.GET_localStorage("SL_LOCALIZATION"),'exttabDict') + " &nbsp; <img src='../../../../content/img/util/speak.png'> " + FExtension.element(FExtension.GET_localStorage("SL_LOCALIZATION"),'extADVTTS')+"</div>";

        GEBI('SL_TR_PROVIDERS_BUILDER').innerHTML=DOMPurify.sanitize(build);
	// init lists 
	initSortable( "list-1" );
...
function SL_ONE_TR_ONLY(st){
	...
	if(st==1){
		for(var i=0; i<items.length; i++){
		    ...
		    if(DOMPurify.sanitize(items[i].innerHTML).indexOf("Google")!=-1) items[i].style.opacity="1";
