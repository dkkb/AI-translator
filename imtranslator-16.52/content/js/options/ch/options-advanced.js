	  ...
	  for(var i=HEADER; i < TMP.length; i++){
		...
		if(localStorage.getItem(LINE[0])!=null){
			...
			var SKIP = 0;
				...
				if(LINE[0].indexOf("_change_lang_HK_it") != -1 || LINE[0].indexOf("_HK_btn") != -1 || LINE[0].indexOf("_HK_gt2") != -1 || LINE[0].indexOf("_HK_it2") != -1 || LINE[0].indexOf("_HK_opt") != -1 || LINE[0].indexOf("_HK_bb2") != -1 || LINE[0].indexOf("_HK_wpt1") != -1 || LINE[0].indexOf("_HK_wpt2") != -1){
					...
					if(CNT==0 || CNT < (HKset.length)) {
						...
						chrome.storage.local.set({'+LINE[0]+': GetFromDefault(LINE[0])});
						SKIP = 1;
					}
					if(CNT>5) SKIP = 1;
					if(SKIP == 1){
				...
				if(LINE[0].indexOf("_Version") != -1){
					...
					if(FExtension.GET_localStorage(GLOB_PREF + "_Version") != curVersion) 	ERRORS = ERRORS + "<br>Installed Vesion : " + FExtension.GET_localStorage(GLOB_PREF + "_Version")+"<br>Restored Backup Vesion : " + curVersion;
					SKIP=1;
				}
