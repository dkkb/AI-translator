...
(function(){GEBI("SRV5").addEventListener("keyup",function(){ if(GEBI("SRV5").value=="")GEBI('SRV5').value=FExtension.GET_localStorage("SL_HK_bb1");},!1); } )();
...
//   SL_MSG(399);
   DO_IFRAME(5);
//   GEBI('SL_kbd').innerText=FExtension.element(FExtension.GET_localStorage("SL_LOCALIZATION"),'extHKconflict');
 }	
}
...
function VERIFY__(ob){
 if(VERIFY_ALL_TABS(5) != true){
    SL_MSG(399);
    if(GEBI('SRV5').value==""){ 
	GEBI('SL_kbd').innerText=FExtension.element(FExtension.GET_localStorage("SL_LOCALIZATION"),'extEnterHK');
    }else{
	GEBI('SL_kbd').innerText=FExtension.element(FExtension.GET_localStorage("SL_LOCALIZATION"),'extHKconflict');
    }	
    SL_MSG(399);
