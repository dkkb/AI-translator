...
function INIT_AUTO_HK(st){
        ...
        box[9]=FExtension.GET_localStorage('SL_tr_hk');
	...
	var T = 0;
	if(FExtension.GET_localStorage("SL_FK_box1") == "true") T = "1";
	box[2]=T+"|"+box[2];

	var T = 0;
	if(FExtension.GET_localStorage("SL_FK_box2") == "true") T = "1";
	box[3]=T+"|"+box[3];

	var T = 0;
	if(FExtension.GET_localStorage("SL_translation_mos_bbl") == "true") T = "1";
	box[4]=T+"|"+box[4];

	var T = 0;
	if(FExtension.GET_localStorage("SL_HK_wptbox1") == "true") T = "1";
	box[5]=T+"|"+box[5];

	var T = 0;
	if(FExtension.GET_localStorage("SL_HK_wptbox2") == "true") T = "1";
	box[6]=T+"|"+box[6];

	var T = 0;
	if(FExtension.GET_localStorage("SL_HK_optbox") == "true") T = "1";
	box[7]=T+"|"+box[7];

	var T = 0;
	if(FExtension.GET_localStorage("SL_HK_btnbox") == "true") T = "1";
	box[8]=T+"|"+box[8];

	var T = 0;
	if(FExtension.GET_localStorage("SL_tr_hk_btn") == "true") T = "1";
	box[9]=T+"|"+box[9];
	...
	TT=0;
	if(st==9){
          if(GEBI('IMT_HK9').checked==true) TT=1;
	  VERbox=TT+"|"+GEBI('SRV9').value;
        }
...
function VERIFY_ALL_TABS(st){
        ...
        if(st==0 || st==1 || st == 2){
	        ...
	        box[1]=FExtension.GET_localStorage('SL_HK_btn');
	        box[2]=FExtension.GET_localStorage('SL_HK_gt1');
        	box[3]=FExtension.GET_localStorage('SL_HK_gt2');
        }
        ...
        if(st==3 || st == 4){
        	...
        	box[6]=GEBI('SRV12').value;
        } else {
	        box[4]=FExtension.GET_localStorage('SL_HK_it1');
        	box[5]=FExtension.GET_localStorage('SL_HK_it2');
        	box[6]=FExtension.GET_localStorage('SL_change_lang_HK_it');
        }

        if(st==5){
        	box[7]=GEBI('SRV5').value;
        } else {
	        box[7]=FExtension.GET_localStorage('SL_HK_bb1');
        }


        if(st==6 || st == 7){
	        box[8]=GEBI('SRV6').value;
        	box[9]=GEBI('SRV7').value;
        } else {
	        box[8]=FExtension.GET_localStorage('SL_HK_wpt1');
        	box[9]=FExtension.GET_localStorage('SL_HK_wpt2');
        }

        
        box[10]=FExtension.GET_localStorage('SL_HK_opt');
       	box[11]=FExtension.GET_localStorage('SL_HK_btn');
	...
	if(box[2]==box[4] || box[2]==box[7] || box[4]==box[7]) cnt++;
